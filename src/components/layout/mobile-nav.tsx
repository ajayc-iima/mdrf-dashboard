      ...
      <div className="flex">
        {nav.map((item) => {
          ...
          const cls = cn(
            ...
            active ? "" : "text-[hsl(var(--text-4))] active:scale-95",
          )
          const inner = (
            <>
              {active && !item.external && (
                <span className="absolute top-0 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-full" style={{ backgroundColor: theme.primary }} />
              )}
              <span className={cn(
              ...
              <span className="text-[10px] font-semibold leading-tight tracking-wide" style={active ? { color: theme.primary } : undefined}>{item.label}</span>
            </>
          )
          if (item.external) {
            return (
