...
import { BarChart3, FileText, Users, BookOpen, TrendingUp } from "lucide-react"
import Link from "next/link"

export default function DataScientistHomePage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-4 stagger-children">
        ...
        <StatCard title="Case Studies" value={cases.length} icon={<BookOpen className="h-4 w-4" />} variant="accent" />
        <StatCard title="MDRF Fellows" value={fellows.length} icon={<Users className="h-4 w-4" />} variant="default" />
        <StatCard title="Need Help" value={silentFellows.length} icon={<TrendingUp className="h-4 w-4" />} variant={silentFellows.length > 0 ? "destructive" : "success"} />
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader className="border-b border-[hsl(var(--border))]">
            <CardTitle className="text-[15px]">Recent Case Studies</CardTitle>
          </CardHeader>
          <CardContent className="pt-4">
            {recentCases.length === 0 ? (
              <p className="py-4 text-center text-[13px] text-[hsl(var(--text-3))]">No case studies yet</p>
            ) : (
            ...
            <Link href="/data-scientist/cases" className="mt-3 block text-center text-[12px] font-semibold text-[hsl(var(--navy))] hover:underline">View all →</Link>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="border-b border-[hsl(var(--border))]">
            <CardTitle className="text-[15px]">Fellows Needing Help</CardTitle>
          </CardHeader>
          <CardContent className="pt-4">
            ...
            <Link href="/data-scientist/fellows" className="mt-3 block text-center text-[12px] font-semibold text-[hsl(var(--navy))] hover:underline">View all fellows →</Link>
          </CardContent>
        </Card>
      </div>
    </div>
