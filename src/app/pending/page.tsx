  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--bg-page))] flex items-center justify-center px-4">
      <div className="w-full max-w-sm text-center">
        <div className={`mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full ${isRejected ? "bg-[hsl(var(--red-soft))]" : "bg-[hsl(var(--gold-soft))]"}`}>
          {isRejected ? <XCircle className="h-7 w-7 text-[hsl(var(--red))]" /> : <Clock className="h-7 w-7 text-[hsl(var(--gold))]" />}
        </div>
        ...
        <div className="rounded-xl border border-[hsl(var(--border))] bg-white p-4 text-left text-[13px] text-[hsl(var(--text-3))] mb-6 shadow-sm">
          {isRejected ? (
            <>Your access request was declined. Contact the administrator if you believe this is an error.</>
          ) : (
            <>
              <p className="mb-2 flex items-center gap-2 font-medium text-[hsl(var(--text-1))]">
